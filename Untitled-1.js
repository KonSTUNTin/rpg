...
class UIManager {
    ...
    openEditor(index) {
        ...
        for (let stat in CARD_DATA.suits) {
            ...
            this.currentUnit.cards[stat].forEach((card, i) => {
                let cardEl = this.createCardElement(card);
                cardEl.onclick = () => {
                    this.currentUnit.cards[stat].splice(i, 1);
